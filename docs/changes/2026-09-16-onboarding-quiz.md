...
- **Same `@waiver-warrior/core` barrel for client and API.** A separate browser entry was rejected. Operator weights now import the JSON file. `CsvProvider.load()` dynamically imports `node:fs` and `csv-parse` so evaluating the barrel in the browser does not pull Node fs. `load()` is still Node-only.
...
On finish, the profile is written to `localStorage` as raw `DraftProfile` JSON under `ww.draftProfile`. Revisit `/onboard` hydrates that into the form from step 1. Invalid JSON is dropped and the quiz restarts from defaults (12 teams, 13 rounds, snake, 9-cat, all Neutral). Skip intensity omits `intensity` from the payload. Rank failure stays on review.
...
- Client store: Zustand persist `ww.draftProfile`.
...
- **Contract:** `DraftProfile` and `POST /rank` are unchanged. Persistence key is `ww.draftProfile` (the profile object, not Zustand's wrapper). 8-cat omits `tov` from `enabledCats`. Punt is `stances[c] = 'punt'`.
...
1. From `/`, Start goes to `/onboard` (no client router). Complete required steps, skip intensity and the archetype pair, Rank my board. Confirm a top 10 and `localStorage.ww.draftProfile` is Zod-valid JSON.
...
4. Put invalid JSON in `ww.draftProfile`, refresh. Quiz restarts at defaults and the key is gone.
