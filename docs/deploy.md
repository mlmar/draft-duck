...
| Image `npm ci` uses `--ignore-scripts` then `npm rebuild esbuild`. `HUSKY=0`.                                                                        | Full lockfile still sees client Vite. Its esbuild 0.25 postinstall dies next to tsx's 0.28 (`Expected "0.25.12" but got "0.28.2"`). Do not drop ignore-scripts. |
